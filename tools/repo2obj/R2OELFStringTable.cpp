...
SString stringToSStringView (std::string const & Str) {
    ...
    auto Ptr = std::shared_ptr<char> (new char[Str.size ()], [](char * P) { delete[] P; });
...
std::uint64_t StringTable::insert (SString const & Name) {
    ...
    if (DidInsert) {
        DEBUG_WITH_TYPE ("repo2obj", (llvm::dbgs () << "  strtab insert "
...
// write
// ~~~~~
std::tuple<std::uint64_t, std::uint64_t> StringTable::write (llvm::raw_ostream & OS) const {
