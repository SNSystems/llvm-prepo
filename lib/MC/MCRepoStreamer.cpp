...
void MCRepoStreamer::ChangeSection(MCSection *Section, const MCExpr *Subsection) {
    MCSymbol * const beginSymbol = Section->getBeginSymbol();
    std::string name = (beginSymbol == nullptr) ? "<none>" : beginSymbol->getName ();
    std::cout << "Repo change section " << name << "\n";

    this->MCObjectStreamer::ChangeSection(Section, Subsection);
}

bool MCRepoStreamer::EmitSymbolAttribute(MCSymbol *Symbol, MCSymbolAttr Attribute) {
    return true; //success
}

void MCRepoStreamer::EmitCommonSymbol(MCSymbol *Symbol, uint64_t Size, unsigned ByteAlignment) {
    std::cout << "Emit common symbol\n";
}

void MCRepoStreamer::EmitZerofill (MCSection *Section, MCSymbol *Symbol, uint64_t Size, unsigned ByteAlignment) {
    std::cout << "Emit zero fill\n";
}

void MCRepoStreamer::EmitInstToData(const MCInst &Inst, const MCSubtargetInfo &STI) {
